...
export async function migrateDbIfNeeded(db: SQLiteDatabase) {
  try {
    ...
    if (currentVersion < 2) await db.execAsync(`CREATE TABLE IF NOT EXISTS budgets (id TEXT PRIMARY KEY NOT NULL, categoryId TEXT NOT NULL, amount REAL NOT NULL, month TEXT NOT NULL, UNIQUE(categoryId, month)); PRAGMA user_version = 2;`);
    ...
    if (currentVersion < 4) {
      try {
        ...
        // Tolerated if column already exists
      }
      await db.execAsync(`PRAGMA user_version = 4;`);
    }
  } catch (err) {
    console.error('Database migration error (tolerated):', err);
  }
}
